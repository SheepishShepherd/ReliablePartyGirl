...
namespace ReliablePartyGirl
{
	...
	public class PartyWorld : ModSystem
	{
		...
		public override void PreUpdateWorld()
		{
			...
			// If a genuine party is occuring and there is no existing Party Girl, attempt to spawn one when ready
			// GenuineParty assumes PartyIsUp is true
			if (BirthdayParty.GenuineParty && NPC.FindFirstNPC(NPCID.PartyGirl) < 0 && SpawnPartyGirlReady) {
				// Spawn a smoke cloud
				for (int j = 0; j < 4; j++) {
